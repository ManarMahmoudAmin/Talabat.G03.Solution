    ...
    public class MappingProfiles : Profile
    {
        public MappingProfiles()
        {
            ...
            CreateMap<Order, OrderToReturnDTO>()
                .ForMember(ordrToReturnDto => ordrToReturnDto.DeliveyMethod, O => O.MapFrom(order => order.DeliveryMethod.ShortName))
                .ForMember(ordrToReturnDto => ordrToReturnDto.DeliveyMethodCoast, O => O.MapFrom(order => order.DeliveryMethod.Cost))
                .ForMember(ordrToReturnDto => ordrToReturnDto.DeliveyMethod, O => O.MapFrom(order => order.DeliveryMethod.ShortName))
                .ForMember(ordrToReturnDto => ordrToReturnDto.DeliveyMethodCoast, O => O.MapFrom(order => order.DeliveryMethod.Cost));
